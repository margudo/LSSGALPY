...
xyplt = [ax1.plot(rad_tot[i][cond_dec(decCen0, decDelta0)[i]], z_tot[i][cond_dec(decCen0, decDelta0)[i]], krgb, ms=mval, alpha=alpval, visible=visi)[0] for i, krgb, mval, alpval, visi in zip(range(0, 4), ['k.', 'ro', 'go', 'bo'], [1, 4, 4, 4], [alpha0, .7, .7, .7], [True, True, False, False])]
...
def update(val):
    ...
    ax2.cla()
    [xyp.set_data(rad_tot[i][cond_dec(decCen, decDelta)[i]], z_tot[i][cond_dec(decCen, decDelta)[i]]) for i, xyp in zip(range(0, 4), xyplt)]
